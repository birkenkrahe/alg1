...
#+begin_src R :file gradesDensity.png :session *R* :results file graphics output :exports both
  ...
  plot(density(grades2), lwd=2, col="blue",
       xlab="Grades [%]", main="Total grades CSC 240 (14 to 30-Sep-2024)",
  ...
  ## add a legend
  legend("topleft", legend=c("Grades1", "Grades2"),
...
1. What should be included in a function documentation header?
   #+begin_quote
   Name, purpose, return type, parameters, edge cases.
   #+end_quote
   Example:
   #+begin_example C
   // Name: power
...
5. Show this with a short example including =main= and a function!
   1) In main: Define 1-dimensional array as {0}
   2) In main: Print array elements
   3) In main: Call function on array
   4) In (void) function: change elements in array to {1}
   5) In main: Print array elements
