...
- Some example projects that I'd like to build:

  1) Build a custom linked-list based *text editor* (command-line based)
     that allows the user to create, edit, and save text files. It
     allows basic operations like inserting, deleting, searching,
     saving text.

  2) Build a *hash table* with collision handling for a dictionary that
     stores words and their definitions, simulating a dictionary. You
     need to implement a hash function, handle collisions, and provide
     search, insert, and delete functionalities.

  3) Design a dynamic array-based *file system* (simulated in-memory)
     that uses a dynamic array (with resizing) to store files and
     directories, simulating basic file system operations such as
     creating, deleting, and listing files and directories, and
     resizing of the storage array as more files are added.

  References: "Do THIS To Master Programming (why coding exercises are
  KILLING your progress!), by Code with Huw (Oct 1, 2024).
