...
- [ ] *Review:* =exit=, =return= and recursion (=power= function)
- [ ] New *quiz* (Oct 25) and new *assignment* (Oct 28)
- [ ] The *Quicksort* algorithm explained & implemented
- [ ] *Coding*: Three practice exercises (Euclid's algorithm)
...
2. Can a =void= function use a =return= statement?
   ...
   #+begin_src C :main no :tangle main.c
     ...
     int main(void) {
       hello((char*){"Marcus"},1); // what's the first argument?
       return 0;
     }
   #+end_src
...
5. What's the process for a recursive function?
   ...
   until it has returned to the first function that called itself.
   #+end_quote
   Example:
   #+begin_example
