...
4. How are =struct= members accessed in C?
   #+BEGIN_QUOTE
   Using the dot operator (=.=), for example, =p1.x= to access the =x=
   member of a =struct Point=.
   #+END_QUOTE
...
6. Why is the =strcpy= function necessary when working with =struct= members that are strings?
   #+BEGIN_QUOTE
...
10. What happens if two =struct= definitions with the same name are declared in different blocks in C?
    #+BEGIN_QUOTE
    There will be no conflict because =struct= names have block scope and
    are local to the block in which they are defined.
    #+END_QUOTE

11. How would you declare and use a =struct= to represent an employee with fields for =name=, =number=, and =salary= in C?
    #+BEGIN_SRC C
      struct Employee {
        char name[30];
        int number;
...
12. What are =argc= and =argv= in the =main= function, and how are they used
    in C programs?
    #+BEGIN_QUOTE
    =argc= is the "argument count," representing the number of
    command-line arguments, including the program name. =argv= is the
...
13. Why would you use =main(int argc, char **argv)= instead of
    =main(void)= in a C program?
    #+BEGIN_QUOTE
    Using =main(int argc, char *argv[])= or =main(int argc, **argv)=
    allows the program to access command-line arguments, enabling
...
Lecture - Review and practice after the break!
