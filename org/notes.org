...
- [ ] *Poll!* Who has got *WSL* (Windows Subsystem Linux) on his Windows
  PC? Enrol my help to do this (same environment as in class)!

- [ ] *New!* 6-minutes podcast on "C functions (see [[https://chat.google.com/room/AAAAcFTK-VI/UifqRr1crOI/UifqRr1crOI?cls=10][chat]]) - let me know
  what you think! (Based on a 46p. script)

- [ ] Three bonus programs available right now (complete by Oct-7) - late
  submission (no later than October 14): 50%.
...
** DONE Programming assignments are getting better!
...
** DONE Review (Passing by value, passing arrays)
...
3. If =*p= is a pointer to =&i=, the address of the =int= variable =i=, what
   ...
   #+RESULTS:
   : i        = 100, &i       = 1827209100, p = 1827209100
   : *p       = 100, &(*p)    = 1827209100
   : *(&i)    = 100
   : *(&(*p)) = 100, &(*(&i)) = 1827209100
